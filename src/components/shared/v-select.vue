...
export default class VSelect extends Vue {
  ...
  @Prop({ default: 'Wybierz opcję' }) placeholder!: string;

  @Prop({ default: false }) clearable!: boolean;

  @Prop({ default: true }) lineStyle!: boolean;

  @Prop() theme!: string;

  @Watch('value') selectedItem(value: any) {
    this.$emit('update:selectedOption', value);
  }

  private selected = this.selectedOption;
  ...
  getValue(option: { [x: string]: any; hasOwnProperty: (arg0: string) => any; }|null) {
    if (option == null) {
      return '';
    }
  ...
  getLabel(option: { [x: string]: any; hasOwnProperty: (arg0: string) => any; }|null) {
    if (option == null) {
      return '';
    }
    if (typeof option === 'object') {

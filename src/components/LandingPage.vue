...
import ContinueQuizModal from '@/components/LandingPage/modals/ContinueQuizModal.vue';
import RecentFolders from '@/components/LandingPage/RecentFolders.vue';
...
const openDialogAsync = (options: any) => new Promise(resolve => dialog.showOpenDialog(options, resolve));
...
export default class LandingPage extends Vue {
  ...
  private isContinueQuizModalOpen = false;
  ...
  private async selectFolder() {
    const path = await openDialogAsync({ properties: ['openDirectory'] }) as string;
  ...
  private async openQuiz(quizPath: any) {
    ...
    if (filenames.find(f => f === 'save.json')) {
      ...
      this.$nextTick(() => {
        const { continueQuizModal } = this.$refs;
        (continueQuizModal as Vue).$on('new', () => this.openNewQuiz(questions, quizPath));
        (continueQuizModal as Vue).$on('continue', () => this.openContinuedQuiz(questions, quizPath));
      });
    } else this.openNewQuiz(questions, quizPath);
  }

  private openNewQuiz(questions: any, quizPath: any) {
    const quiz = quizMaker.prepareQuizObject(questions, quizPath);
    this.goToQuiz(quiz, quizPath);
  }

  private async openContinuedQuiz(questions: any, quizPath: any) {
    const quiz = await quizMaker.prepareQuizObjectWithSave(questions, quizPath);
    this.goToQuiz(quiz, quizPath);
  }

  private goToQuiz(quiz: any, quizPath: any) {

    ...
    <div class="quiz-info-wrapper">
      ...
      <div class="quiz-info-wrapper-content">
        ...
        <div class="buttons">
          ...
          </button><button v-if="quiz.location" @click="saveQuiz"><i class="fal fa-arrow-from-top faSave"/>
          </button><button @click="$emit('showSettings')"><i class="fal fa-arrow-from-top faCog"/>
          </button><button @click="$emit('showInfo')"><i class="fal fa-arrow-from-top faInfo"/></button>
        </div>
      </div>
    </div>
  </div>
...
import FinishQuizModal from '@/components/Quiz/modals/FinishQuizModal.vue';
import SaveBeforeExit from '@/components/Quiz/modals/SaveBeforeExit.vue';
import SelectOptionsModal from '@/components/Quiz/modals/SelectOptionsModal.vue';
...
export default class Quiz extends Vue {
  ...
  private acceptVisible = true;

  private currentQuestionTag: any = null;

  private currentQuestion: any = null;

  private unsortedAnswers: any = null;

  private quiz = this.quizObject;

  private showFinishModal = false;

  private isSelectOptionsModalOpened = false;

  private isSaveBeforeQuitQuizModalOpened = false;

  private isSaveBeforeQuitAppModalOpened = false;

  private answers: any = [];

  private selectAnswers: any = [];

  private selectObject = Object;

  private selectOptionsModalContent = null;
  ...
  private checkUserAnswer() {
    if (this.currentQuestionTag) {
      const questionReoccurrences = this.quiz.reoccurrences.find((r: any) => r.tag === this.currentQuestionTag);
      ...
      if (this.currentQuestion.type === 'single') {
        ...
        isCorrect = !this.selectAnswers.find((c: any) => c.correctOptionId !== c.selectedOptionId);
      }
    ...
    const remainingQuestionsReoccurrences = this.quiz.reoccurrences.filter((r: any) => r.value > 0);
  ...
  private randomQuestion() {
    ...
    if (remainingQuestionsReoccurrences.length) {
      ...
      if (questionTag) {
        ...
        this.currentQuestion = this.quiz.questions.find((q: any) => q.tag === this.currentQuestionTag);
        if (this.currentQuestion.type === 'single') {
          this.unsortedAnswers = _.shuffle(this.currentQuestion.answers);
        } else if (this.currentQuestion.type === 'select') {
          this.selectAnswers = this.currentQuestion.answers.map((s: any) => ({
            selectId: s.id,
  ...
  private setSelectedOptionId(response: any) {
    const item = this.selectAnswers.find((i: any) => i.selectId === response.selectId);
    if (item != null) {
      item.selectedOptionId = response.optionId;
    }
  }

  private showSelectOptionsModal(selectId: string) {
    this.isSelectOptionsModalOpened = true;
    this.selectOptionsModalContent = this.currentQuestion.answers.find((s: any) => s.id === selectId);
  }

  private toggleAnswerSelect(index: number) {
    if (index >= 0 && index < this.unsortedAnswers.length) {
      const { id } = this.unsortedAnswers[index];
  ...
  private reoccurrencesIfBad() {
    return this.$store.state.reoccurrencesIfBad;
  }

  private maxReoccurrences() {
    return this.$store.state.maxReoccurrences;
  }

  private selectQuestionContent() {
    if (this.currentQuestion == null || this.currentQuestion.type !== 'select') {
      return null;
    }
    const cont = this.unsortedAnswers.map((l: any) => {
      if (typeof l !== 'string') {
        const selection = this.selectAnswers.find((i: any) => i.selectId === l.selectId);
        if (selection.selectedOptionId != null) {
          const select = this.currentQuestion.answers.find((s: any) => s.id === l.selectId);
          if (select != null) {
            const option = select.options.find((o: any) => o.id === selection.selectedOptionId);
  ...
  private formatMoment(date: number) {
